...
export default function SubmissionDetailsModal({
  ...
  const [attachmentsToDelete, setAttachmentsToDelete] = useState<Set<string>>(
    new Set()
  )
  ...
  const handleSaveAttachments = async () => {
    ...
    if (attachmentsToDelete.size > 0) {
      await Promise.all(
        Array.from(attachmentsToDelete).map((id) =>
          deleteSubmissionAttachment(id)
        )
      )
    }
  ...
  const hasAttachmentChanges =
    pendingFiles.length > 0 || attachmentsToDelete.size > 0
    ...
    <Modal isOpen={isOpen && !!submission} close={onClose}>
      ...
      <Modal.Body>
        ...
        <div className="flex flex-col gap-y-2 mt-2">
          <DetailRow label="Bug Description">
            {submission?.bug?.description}
          </DetailRow>
        ...
        {submission && (
          <div
            className="mt-4 border-t pt-4 flex flex-col"
            style={{ height: "300px" }}
          >
