...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  const bug_id = req.query.bug_id as string
  ...
  const { result } = await uploadFilesWorkflow(req.scope).run({
    input: {
      files: files.map((f) => ({
        ...
        access: "public", // or "private" if attachments should be restricted
      })),
    },
  })

  // Now persist the attachment records
  const service: BugTrackerModuleService = req.scope.resolve(BUGTRACKER_MODULE)

  const attachments = await service.createBugAttachments(
    result.map((f) => ({
      file_id: f.id,
      file_url: f.url,
      filename: f.id,
