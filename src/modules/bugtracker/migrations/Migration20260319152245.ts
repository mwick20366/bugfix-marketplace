...
export class Migration20260319152245 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table if not exists "developer" ("id" text not null, "firstName" text not null, "lastName" text not null, "email" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "developer_pkey" primary key ("id"));`);
    this.addSql(`CREATE INDEX IF NOT EXISTS "IDX_developer_deleted_at" ON "developer" ("deleted_at") WHERE deleted_at IS NULL;`);

    this.addSql(`create table if not exists "bug" ("id" text not null, "title" text not null, "description" text not null, "techStack" text not null, "repoLink" text null, "bounty" real null, "status" text null, "client_id" text not null, "developer_id" text null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "bug_pkey" primary key ("id"));`);
